...
RSpec.describe Kaal::Sinatra, integration: :pg do
  include SinatraIntegrationHelpers

  it 'integrates a classic Sinatra app through the postgres backend' do
    ...
    run_dummy_app(
      ...
      expect(lines).to eq(['200', 'Kaal::Backend::Postgres', 'true'])
  ...
  it 'integrates a modular Sinatra app through the postgres backend' do
    skip 'DATABASE_URL not set' if ENV['DATABASE_URL'].to_s.empty?
