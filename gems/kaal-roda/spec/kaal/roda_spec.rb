...
RSpec.describe Kaal::Roda do
  ...
  it 'has a version number and exposes the Roda plugin' do
    expect(described_class::VERSION).to eq('0.6.1')
  ...
  it 'builds a redis backend when a redis client is provided' do
    ...
    Kaal.configuration.namespace = 'roda-redis'
  ...
  it 'raises when backend inference is unsupported without an explicit adapter override' do
    ...
    expect { described_class.configure_backend!(database: unsupported_db) }
      .to raise_error(ArgumentError, /Unsupported Roda datastore backend; use memory, redis, sqlite, postgres, or mysql/)
  end

  it 'registers the app, loads scheduler definitions, and does not auto-start by default' do
    Dir.mktmpdir('kaal-roda-register-') do |root|
      ...
      stub_const('KaalRodaSpecJob', Class.new do
        def self.perform(*); end
      end)

      fake_app_class.opts[:root] = root
      fake_app_class.opts[:environment] = :test
      database = Sequel.sqlite(database_path)
      KaalRodaDummyAppSupport.create_sql_schema(database, 'sqlite')
  ...
  it 'preserves the default scheduler path when register! is called with nil' do
    fake_app_class.opts[:root] = Dir.pwd
  ...
  it 'loads scheduler files with fallback environment detection' do
    Dir.mktmpdir('kaal-roda-load-') do |root|
      ...
      stub_const('KaalRodaLoadJob', Class.new do
        def self.perform(*); end
      end)
      ...
      expect(Kaal.registered?(key: 'roda:load')).to be(true)
    ensure
  ...
  it 'resolves private helper fallbacks for plain objects' do
    ...
    expect(described_class.send(:root_path_for, plain_app, root: '/tmp/kaal-roda')).to eq(Pathname.new('/tmp/kaal-roda'))
  ...
  it 'installs the roda class DSL through the plugin' do
    app = Class.new(Roda)
    database = Sequel.sqlite
  ...
  it 'avoids duplicate shutdown hook registration and swallows stop errors in the managed hook' do
    ...
    expect(logger).to have_received(:error).with(/Failed to stop Kaal during Roda shutdown: stop failure/)
  end
