  ...
  class SchedulerFileLoader
    ...
    class JobApplier
      ...
      def persisted_metadata(job)
        ...
        normalized_metadata = @helper_bundle.stringify_keys(metadata.deep_dup)
        normalized_metadata.deep_merge(
          'execution' => {
            'target' => 'active_job',
      ...
      def build_callback(job)
        ...
        lambda do |fire_time:, idempotency_key:|
          ...
          resolved_args = @helper_bundle.resolve_placeholders(args_template.deep_dup, context)
          raw_kwargs = @helper_bundle.resolve_placeholders(kwargs_template.deep_dup, context) || {}
