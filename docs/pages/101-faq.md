...
- use `kaal` with `sequel` or `activerecord` in plain Ruby and configure the backend yourself
- use `kaal` with `sequel` or `activerecord` in plain Ruby and configure the backend yourself
...
- `config/kaal.yml` exists and loads cleanly
- the required gems for your selected backend are installed
- `KAAL_BACKEND_URL`, `REDIS_URL`, or `DATABASE_URL` is present when your setup depends on it
- the configured `backend` and `backend_config` match the connection you are passing

## Why are no jobs loading?

Check:

- `config/kaal-scheduler.yml` exists at the configured path
